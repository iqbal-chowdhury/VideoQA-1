...
class SeqVladWithReduAttentionModel(object):
	...
	def init_parameters(self):
		...
		self.liner_W = tf.get_variable("liner_W",[self.centers_num*self.reduction_dim, self.d_w2v],
				initializer=tf.random_normal_initializer(stddev=1. / math.sqrt(self.centers_num*self.reduction_dim)))

		self.liner_b = tf.get_variable("liner_b",initializer=tf.random_normal([self.d_w2v],stddev=1./math.sqrt(self.d_w2v)))
	...
	def encoder(self):
		...
		embeding_output = tf.nn.xw_plus_b(tf.reduce_mean(vlad,axis=1),self.liner_W, self.liner_b)
		return embeding_output
	
	def build_model(self):
		print('building seq model ... ...')
		self.init_parameters()
		embeding_output = self.encoder()
		return embeding_output
